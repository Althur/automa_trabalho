...
def build_btc_contract_info(btc_elements):
    ...
    for item in range(len(elements)):
        ...
        obj = {
            "position_date": date_of_position, # format:
            "lender_borrower": lender_borrower, 
            "ticker_symbol": ticker_symb, 
            "fund_account_number": fund_acc_number, 
            "contract_number": contract_id,
            "register_date": date_of_trade, 
            "previous_liquidation_quantity": liquidation_qnty, 
            "expire_date": expiration_date, 
            "due_date": due_date,
            "test_element": test_element,
            "quantity": contract_quantity,
            "net_present_quantity": net_contract_quantity, 
            "wallet_number": portfolio_id, # carteira planilha margem novo
            "underlying_asset_price": referenc_price_underlying_asset,  # ver na planilha margem novo
            "contract_interest_rate": contract_interest,
            "counterparty": counterparty, # contra parte
            "brokerage_firm": broker_number, # corretora
            "daily_cost": '', # custo diario
            "days_to_exp": '', # dias até vcto _> formula business days
            "margin_call": '', # chamada_de margem
            "fund_nickname": nick
        }
...
def segmenta_imbarq(lines, target_group_list, target_range=[0,2]):
  ...
  for i in range(len(lines)):
    #print(lines[i])
    line = str(lines[i], 'ISO-8859-1')
    #line = str(line, 'utf-8')
    if line[target_range[0]:target_range[1]] == target_list[0]:
      print("make money")
    for j in range(len(target_list)):
      if line[target_range[0]:target_range[1]] == target_list[j]:
        group_list[j].append(line) # add à lista de grupos cada segmento do IMBARQ001 separado individualmente  
        #print("pump it up")
